...
#This is not the code we used, use chunk below for FIPS id 
all_data <- list()
for (yr in 2000:2024) {
...
#Making the weather data times to become more tidy 
sf_weather <- sf_weather %>% 
  mutate(date_time = ymd_hms(date),
         date_new = as_date(date_time))

#plotting the weather
ggplot(sf_weather, aes(x=date_new, y = temp_f, color = datatype)) + geom_line() +
  scale_x_date(
    breaks=seq(from = as.Date("2000-01-01"), to = as.Date("2025-01-01"), by = "5 years"),
    date_labels = "%Y" #only showing the year
  ) +
  labs (x = "Years", y = "Temperature(Fahrenheit)", title = "Daily Min and Max Temps throughout SF")
```
